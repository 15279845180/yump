...
/**
 * 生成 CDR 按天总数汇总表 DDL（极简结构：仅 date + total_count）
 * 无条件查询时直接 SUM(total_count) 取总数，O(天数) 毫秒级，绝不对日表 COUNT(*)
 * 与 y_cdr_counter 的区别：去掉 node_id 维度，只维护当日全局总数，查询更简单
 */
function getSummaryDDL(tableName = 'y_cdr_summary') {
  return `
    CREATE TABLE IF NOT EXISTS \`${tableName}\` (
      \`date\`        DATE          NOT NULL COMMENT '日期(YYYY-MM-DD)',
...
module.exports = { TABLE_PREFIX, getCdrTableName, getCdrDDL, getCounterDDL, getHourlyDDL, getDailyDimDDL, getSummaryDDL, getDateRangeTables }
