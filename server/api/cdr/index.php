...
function handleList($pdo) {
    ...
    // duration/disconnect_cause/cdr_id 等字段过滤。node_id 可选（counter 含 node_id 列）。
    // 无字段筛选时走 counter 分段定位分页（见下），深翻页毫秒级，不限制 offset；
    // 带字段筛选时 counter 无法预聚合，精确 COUNT 深分页慢，保留 5000 上限防滥用。
    ...
    // 深度限制：仅「带字段筛选」时生效（精确 COUNT 深分页慢）。
    // 无筛选走 counter 分段定位，localOffset 永远 < 单天总数，深翻页仍是毫秒级，不限制。
    ...
    if ($noFieldFilter) {
        // —— 总数 + 分段定位：预聚合计数器 ——
        // 1) 从 y_cdr_counter 取每日 (date, total) 倒序（O(天数)，毫秒级）
        // 2) 计算前缀和定位 offset 落在哪一天 → 该天局部偏移 localOffset
        // 3) 只查 1~2 个日表的局部 LIMIT（跨界时补前一天头部），避免全表扫描/深分页
        //    这是「无筛选任意页码毫秒级」的核心：localOffset 永远 < 单天总数，
        //    第 1 页和第 10000 页的查询代价完全相同（都只扫 1~2 个日表的小 LIMIT）。
        $dateList = [];
        ...
        $total = 0;
        $counterMap = []; // date(YYYY-MM-DD) => 当日总数
        if ($dateList) {
            $ph = implode(',', array_fill(0, count($dateList), '?'));
            $cSQL = "SELECT date, SUM(total) AS day_total FROM y_cdr_counter WHERE date IN ($ph)";
            $cParams = $dateList;
            if (!empty($_GET['node_id'])) {
                $cSQL .= " AND node_id = ?";
                $cParams[] = intval($_GET['node_id']);
            }
            $cSQL .= " GROUP BY date ORDER BY date DESC";
            $stmt = $pdo->prepare($cSQL);
            $stmt->execute($cParams);
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $counterMap[$row['date']] = intval($row['day_total']);
                $total += intval($row['day_total']);
            }
        }

        // 分段定位：找 targetDay + localOffset
        $datesDesc = array_keys($counterMap); // 已倒序（ORDER BY date DESC）
        $targetDate = null;
        $localOffset = 0;
        $accum = 0;
        foreach ($datesDesc as $d) {
            if ($accum + $counterMap[$d] > $offset) {
                $targetDate = $d;
        ...
        if ($localOffset + $pageSize <= $targetDayTotal) {
            // 单表：直接局部 LIMIT（走 idx_received_at，毫秒级）
            $dataSQL = $branch($targetTable) . " ORDER BY c.received_at DESC LIMIT {$localOffset}, {$pageSize}";
            $dataParams = $params;
        } else {
            // 跨界：targetDay 剩余 + 前一天头部，两天结果天然有序（targetDay 新在前）直接拼接
            $needFromPrev = $localOffset + $pageSize - $targetDayTotal;
            $idx = array_search($targetDate, $datesDesc);
            $prevDate = $datesDesc[$idx + 1] ?? null;
            if ($prevDate) {
                $prevTable = 'y_cdr_' . str_replace('-', '', $prevDate);
                $b1 = "(SELECT {$cols} FROM `{$targetTable}` c " . cdrJoin() . " WHERE {$whereSQL} ORDER BY c.received_at DESC LIMIT {$localOffset}, " . ($targetDayTotal - $localOffset) . ")";
                $b2 = "(SELECT {$cols} FROM `{$prevTable}` c " . cdrJoin() . " WHERE {$whereSQL} ORDER BY c.received_at DESC LIMIT {$needFromPrev})";
                $dataSQL = $b1 . " UNION ALL " . $b2;
                $dataParams = array_merge($params, $params);
            } else {
                // 没有前一天（targetDay 是最早一天），只取剩余
                $dataSQL = $branch($targetTable) . " ORDER BY c.received_at DESC LIMIT {$localOffset}, " . ($targetDayTotal - $localOffset);
                $dataParams = $params;
            }
        }
    } else {
        // —— 带字段筛选：counter 无法预聚合，精确 COUNT(*) ——
        ...
        $dataSQL = "SELECT * FROM ({$union}) t ORDER BY received_at DESC LIMIT {$offset}, {$pageSize}";
...
function handleExport($pdo) {
    ...
    // 构建 SELECT 列（加 received_at 做排序，输出时跳过）
    $hasReceivedAt = in_array('received_at', $selected);
    $selectCols = implode(', ', array_map(function ($f) use ($joinCols) {
        return $joinCols[$f] ?? "c.{$f}";
    }, $selected));
    if (!$hasReceivedAt) {
        $selectCols .= ', c.received_at AS _sort';
    }
    ...
    $maxRows = 100000;
    if (count($tables) === 1) {
        $sql = $branch($tables[0]) . " ORDER BY c.id DESC LIMIT {$maxRows}";
    } else {
        $union = implode(' UNION ALL ', array_map($branch, $tables));
        $sortField = $hasReceivedAt ? 'received_at' : '_sort';
...
function handleClear($pdo) {
    ...
    foreach ($tables as $t) {
        ...
        // 同步清预聚合计数器：counter 由 cdr-receiver 增量累加，日表被清空时若不同步清，
        // 会导致无筛选查询的总数严重失真（这正是此前 counter 与日表漂移的根因）。
        if (preg_match('/y_cdr_(\d{4})(\d{2})(\d{2})/', $t, $m)) {
            $stmt = $pdo->prepare("DELETE FROM y_cdr_counter WHERE date = ?");
            $stmt->execute(["{$m[1]}-{$m[2]}-{$m[3]}"]);
        }
        $count++;
    }
